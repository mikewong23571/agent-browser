...
This opens a visible browser window instead of running headless. Also configurable via `AGENT_BROWSER_HEADED=true`.
...
The `--allow-file-access` flag adds Chromium flags that allow `file://` URLs to load and render local files, access other local files via JavaScript, and load local resources (images, scripts, stylesheets).
